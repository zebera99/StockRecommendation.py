...
SPY_year = web.get_data_yahoo('SPY',
...
def price(ticker, months):
  return round(
    web.get_data_yahoo(ticker, start=months)['Close'].head(1).iloc[0], 2)
...
SPY_momentum_score = round(
  (SPY_profit1 * 12 + SPY_profit3 * 4 + SPY_profit6 * 2 + SPY_profit12) / 100,
  3)
...
VEA_momentum_score = round(
  (VEA_profit1 * 12 + VEA_profit3 * 4 + VEA_profit6 * 2 + VEA_profit12) / 100,
  3)
...
VWO_momentum_score = round(
  (VWO_profit1 * 12 + VWO_profit3 * 4 + VWO_profit6 * 2 + VWO_profit12) / 100,
  3)
...
BND_momentum_score = round(
  (BND_profit1 * 12 + BND_profit3 * 4 + BND_profit6 * 2 + BND_profit12) / 100,
  3)
...
QQQ_momentum_score = round(
  (QQQ_profit1 * 12 + QQQ_profit3 * 4 + QQQ_profit6 * 2 + QQQ_profit12) / 100,
  3)
...
EFA_momentum_score = round(
  (EFA_profit1 * 12 + EFA_profit3 * 4 + EFA_profit6 * 2 + EFA_profit12) / 100,
  3)
...
SHY_momentum_score = round(
  (SHY_profit1 * 12 + SHY_profit3 * 4 + SHY_profit6 * 2 + SHY_profit12) / 100,
  3)
...
IEF_momentum_score = round(
  (IEF_profit1 * 12 + IEF_profit3 * 4 + IEF_profit6 * 2 + IEF_profit12) / 100,
  3)
...
TLT_momentum_score = round(
  (TLT_profit1 * 12 + TLT_profit3 * 4 + TLT_profit6 * 2 + TLT_profit12) / 100,
  3)
...
TIP_momentum_score = round(
  (TIP_profit1 * 12 + TIP_profit3 * 4 + TIP_profit6 * 2 + TIP_profit12) / 100,
  3)
...
LQD_momentum_score = round(
  (LQD_profit1 * 12 + LQD_profit3 * 4 + LQD_profit6 * 2 + LQD_profit12) / 100,
  3)
...
HYG_momentum_score = round(
  (HYG_profit1 * 12 + HYG_profit3 * 4 + HYG_profit6 * 2 + HYG_profit12) / 100,
  3)
...
BWX_momentum_score = round(
  (BWX_profit1 * 12 + BWX_profit3 * 4 + BWX_profit6 * 2 + BWX_profit12) / 100,
  3)
...
EMB_momentum_score = round(
  (EMB_profit1 * 12 + EMB_profit3 * 4 + EMB_profit6 * 2 + EMB_profit12) / 100,
  3)

#안전 자산(12개월 평균가)

TIP_12_average = round(
  web.get_data_yahoo(
    'TIP', start=today - timedelta(days=364), end=today)['Close'].sum() / len(
      web.get_data_yahoo('TIP', start=today - timedelta(days=364),
                         end=today)['Close']), 2)

DBC_12_average = round(
  web.get_data_yahoo(
    'DBC', start=today - timedelta(days=364), end=today)['Close'].sum() / len(
      web.get_data_yahoo('DBC', start=today - timedelta(days=364),
                         end=today)['Close']), 2)

BIL_12_average = round(
  web.get_data_yahoo(
    'BIL', start=today - timedelta(days=364), end=today)['Close'].sum() / len(
      web.get_data_yahoo('BIL', start=today - timedelta(days=364),
                         end=today)['Close']), 2)

IEF_12_average = round(
  web.get_data_yahoo(
    'IEF', start=today - timedelta(days=364), end=today)['Close'].sum() / len(
      web.get_data_yahoo('IEF', start=today - timedelta(days=364),
                         end=today)['Close']), 2)

TLT_12_average = round(
  web.get_data_yahoo(
    'TLT', start=today - timedelta(days=364), end=today)['Close'].sum() / len(
      web.get_data_yahoo('TLT', start=today - timedelta(days=364),
                         end=today)['Close']), 2)

LQD_12_average = round(
  web.get_data_yahoo(
    'LQD', start=today - timedelta(days=364), end=today)['Close'].sum() / len(
      web.get_data_yahoo('LQD', start=today - timedelta(days=364),
                         end=today)['Close']), 2)

BND_12_average = round(
  web.get_data_yahoo(
    'BND', start=today - timedelta(days=364), end=today)['Close'].sum() / len(
      web.get_data_yahoo('BND', start=today - timedelta(days=364),
                         end=today)['Close']), 2)

SPY_12_average = round(
  web.get_data_yahoo(
    'SPY', start=today - timedelta(days=364), end=today)['Close'].sum() / len(
      web.get_data_yahoo('SPY', start=today - timedelta(days=364),
                         end=today)['Close']), 2)
...
#BAA 카나리아 자산군
canary_universe = [
  SPY_momentum_score, VEA_momentum_score, VWO_momentum_score,
  BND_momentum_score
]
#BAA 공격자산
offensive_universe = [
  VEA_momentum_score, VWO_momentum_score, BND_momentum_score,
  QQQ_momentum_score
]
#BAA 안전 자산
defensive_universe = [
  TIP_12_average_percent, DBC_12_average_percent, BIL_12_average_percent,
  IEF_12_average_percent, TLT_12_average_percent, LQD_12_average_percent,
  BND_12_average_percent
]
#듀얼 모멘텀 안전 자산
dual_momentum_defensive_universe = [
  SHY_profit12, IEF_profit12, TLT_profit12, TIP_profit12, LQD_profit12,
  HYG_profit12, BWX_profit12, EMB_profit12
]
#채권
bond_universe = [
  SHY_profit6, IEF_profit6, TLT_profit6, TIP_profit6, LQD_profit6, HYG_profit6,
  BWX_profit6, EMB_profit6
]
...
if SPY_momentum_score > 0 and VEA_momentum_score > 0 and VWO_momentum_score > 0 and BND_momentum_score > 0:
  ...
  if biggest == TIP_12_average_percent:
    if biggest < 1:
      ...
      print(
        f'You should buy {int((BAA_money/3) / price("tip",today))} TIP stocks')
    ...
    if biggest < 1:
      ...
      print(
        f'You should buy {int((BAA_money/3) / price("dbc",today))} DBC stocks')
    ...
    if biggest < 1:
      ...
      print(
        f'You should buy {int((BAA_money/3) / price("bil",today))} BIL stocks')
    ...
    if biggest < 1:
      ...
      print(
        f'You should buy {int((BAA_money/3) / price("ief",today))} IEF stocks')
    ...
    if biggest < 1:
      ...
      print(
        f'You should buy {int((BAA_money/3) / price("lqd",today))} LQD stocks')
    ...
    if biggest < 1:
      ...
      print(
        f'You should buy {int((BAA_money/3) / price("bnd",today))} BND stocks')
  ...
  if second_biggest == TIP_12_average_percent:
    ...
    if second_biggest < 1:
      ...
      print(f'TLT - {TLT_12_average_percent}')
      print(
        f'You should buy {int((BAA_money/3) / price("tlt",today))} TLT stocks')
...
if SPY_profit12 > 0:
  ...
  if biggest == SHY_profit12:
    if biggest < 0:
      ...
      print(
        f'You should buy {int((dual_momentum_money/3) / price("shy",today))} SHY stocks'
      )
    ...
    if biggest < 0:
      ...
      print(
        f'You should buy {int((dual_momentum_money/3) / price("ief",today))} IEF stocks'
      )
    ...
    if biggest < 0:
      ...
      print(
        f'You should buy {int((dual_momentum_money/3) / price("tlt",today))} TLT stocks'
      )
    ...
    if biggest < 0:
      ...
      print(
        f'You should buy {int((dual_momentum_money/3) / price("tip",today))} TIP stocks'
      )
    ...
    if biggest < 0:
      ...
      print(
        f'You should buy {int((dual_momentum_money/3) / price("lqd",today))} LQD stocks'
      )
    ...
    if biggest < 0:
      ...
      print(
        f'You should buy {int((dual_momentum_money/3) / price("hyg",today))} HYG stocks'
      )
    ...
    if biggest < 0:
      ...
      print(
        f'You should buy {int((dual_momentum_money/3) / price("bwx",today))} BWX stocks'
      )
    ...
    if biggest < 0:
      ...
      print(
        f'You should buy {int((dual_momentum_money/3) / price("emb",today))} EMB stocks'
      )
...
print('bond:')
bond_universe.sort()
biggest = max(bond_universe)
second_biggest = bond_universe[-2]
third_biggest = bond_universe[-3]
...
#채권 수정하기. 아직 시작 안함 universe 부분만 끝냄
